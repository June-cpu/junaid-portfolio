...
export const LECTURES: Lecture[] = [
  ...
  // ─────────────────────────────── LESSON 9 ────────────────────────────────
  {
    ...
    slides: [
      ...
      {
        type: 'concept',
        label: '— Sensor Numbers',
        heading: 'What Numbers Does the Sensor Return?',
        body: 'The color sensor does not just say "black" or "white" — it returns a NUMBER. Understanding these numbers is key to writing good line-following code:',
        items: [
          '🔢 The sensor returns a value from 0 to 100 — think of it like a percentage of brightness.',
          '⚫ Pure black (on the line): reads roughly 5–15. Very little light bounces back.',
          '⚪ Pure white (off the line): reads roughly 80–95. Lots of light bounces back.',
          '🩶 The edge (half-on, half-off): reads roughly 40–60. This is the sweet spot for edge tracking!',
          '🎯 THRESHOLD: you pick one number (e.g. 50) — below it means "turn left", above it means "turn right".',
          '⚠️ Lighting matters! Bright sun or dim rooms shift all readings. Always test in the same light conditions you will race in.',
        ],
      },
      {
        type: 'visual',
      ...
      },
      {
        ...
        heading: 'Why Do Line Followers Fail? Common Mistakes',
        body: 'Before you build, learn the most common reasons line followers go off track — and how to fix them:',
        items: [
          '🏎️ TOO FAST: The robot moves faster than the sensor can react. Fix: slow down first, then speed up once the code works.',
          '📏 SENSOR TOO HIGH: A sensor far from the ground reads a blurry mix of colors. Fix: mount the sensor as close to the floor as possible.',
          '🎯 WRONG THRESHOLD: If your cutoff number is too high or too low, the robot always turns one way. Fix: read the actual sensor values first, then set the threshold in the middle.',
          '🔄 OVER-CORRECTING: Big corrections cause the robot to zigzag wildly. Fix: use small motor speed differences — gentle nudges beat hard turns.',
          '🔬 CHANGING TOO MANY THINGS AT ONCE: You tweak speed AND threshold AND sensor height — now you do not know what helped. Fix: change ONE variable at a time.',
          '💡 PRO TIP: Add a print statement to display sensor readings while the robot runs — seeing real numbers makes debugging much easier.',
        ],
      },
      {
        type: 'visual',
      ...
      },
      {
        ...
        heading: 'Where Does This Take You?',
        body: 'The feedback loop you are programming today is the same idea used by professionals in some of the most exciting careers in the world:',
        cards: [
          { icon: '🤖', title: 'Robotics Engineer', body: 'Programs industrial robots to follow paths on factory floors, assemble products, and navigate warehouses safely.' },
          { icon: '🚗', title: 'Self-Driving Car Engineer', body: 'Designs sensor + feedback systems that keep autonomous cars in their lanes, just like your robot follows a line.' },
          { icon: '🛸', title: 'Control Systems Engineer', body: 'Builds the feedback loops that keep rockets on course, planes stable in turbulence, and satellites pointing at Earth.' },
          { icon: '💻', title: 'Embedded Software Developer', body: 'Writes the tight, fast IF/ELSE loops that run inside real robots, medical devices, and smart home systems.' },
        ],
      },
      {
        type: 'project',
      ...
      },
      {
        type: 'concept',
        label: '— Wrap Up',
        heading: "Today's Big Ideas — Exit Ticket",
        body: "Before we clean up, let's make sure these key ideas are locked in. Can you answer each one?",
        items: [
          '🔢 What range of numbers does the color sensor return, and what do the extremes represent?',
          '🎯 What is a THRESHOLD and how do you use it to decide which way to turn?',
          '🔁 Explain a feedback loop in one sentence using sense → react → repeat.',
          '📐 Why is tracking the EDGE of a line better than tracking the CENTER?',
          '🐛 Name TWO common mistakes that cause a line follower to fail — and their fixes.',
          '🌍 Name ONE real-world machine or system that uses a feedback loop similar to your robot.',
        ],
      },
    ],
  },
